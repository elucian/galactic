...
const GameEngine: React.FC<GameEngineProps> = ({ ships, shield, secondShield, onGameOver, difficulty, currentPlanet, quadrant }) => {
  ...
  const stateRef = useRef({
    ...
    charging: { active: false, startTime: 0, level: 0 }
  });
  ...
  const drawShip = (ctx: CanvasRenderingContext2D, sInst: any, x: number, y: number, thrust: number, side: number, isBreaking: boolean, rotation: number = 0, isGhost: boolean = false) => {
    ...
    // Gun Glow if Charging
    if (chargeLevel > 0) {
        ctx.shadowBlur = 15 + chargeLevel * 25;
        ctx.shadowColor = '#fff';
    }
  ...
  // Reusable function for firing weapons
  const fireWeapons = (multiplier: number = 1.0) => {
    const s = stateRef.current;
    ...
    s.equippedWeapons.forEach((w) => {
        ...
        // Auto-fire checks only needed for normal multiplier (1.0). Overload shots force fire.
        if (multiplier === 1.0 && now - s.lastFire < (weaponDef.fireRate > 30 ? 0 : (1000 / weaponDef.fireRate))) return;

        const gunPositions = activeShip.config.defaultGuns === 1 ? [0] : [-18, 18];
        
        gunPositions.forEach((xOff, i) => {
            // Apply multiplier to velocity, size, and damage
            const sizeMult = 1 + (multiplier - 1) * 0.3; // Thicker if charged
            const speedMult = 1 + (multiplier - 1) * 0.5; // Faster if charged
            ...
            if (activeShip.config.defaultGuns > 1) {
                ...
                if (xOff < 0) baseB.vx = -baseB.vy * Math.tan(spreadAngle * -1); 
                else baseB.vx = -baseB.vy * Math.tan(spreadAngle);
            } else {
                const maxAngle = 0.2; // +/- 11 degrees
                baseB.vx = -baseB.vy * Math.tan((Math.random() - 0.5) * maxAngle);
            }

            // Weapon specific overrides (scaled by charge if needed)
            if (weaponDef.id === 'exotic_fan') { 
                baseB.vx *= 2.5; 
            } 
            
            if (weaponDef.id === 'exotic_flame') {
                baseB.life = 25 * (1 + multiplier * 0.2); // Longer life for charged flame
                baseB.vy = -18 * speedMult;
        ...
        audioService.playWeaponFire(weaponDef.type === WeaponType.LASER ? 'laser' : 'cannon'); 
        fired = true;
    });
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent, isDown: boolean) => { 
        ...
        if (['Tab', 'CapsLock', 'ShiftLeft', 'Space'].includes(e.code)) {
            e.preventDefault();
        }

        if (isDown) {
            // SPACEBAR: Start Charge
            if (e.code === 'Space' && !s.keys.has('Space')) {
                s.charging.active = true;
                s.charging.startTime = Date.now();
                s.charging.level = 0;
            }
            s.keys.add(e.code); 
        } else {
            // SPACEBAR: Release to Fire Charged Shot
            if (e.code === 'Space' && s.keys.has('Space')) {
                if (s.charging.active) {
                    const duration = Date.now() - s.charging.startTime;
                    const maxChargeTime = 1500; // 1.5s for max charge
                    // Multiplier scales from 1x to 10x based on duration
                    let multiplier = 1.0;
                    if (duration > 200) {
                        const ratio = Math.min(1, duration / maxChargeTime);
                        multiplier = 1 + (ratio * 9); // 1 to 10
                    }
                    fireWeapons(multiplier);
    ...
    const loop = () => {
      ...
      if (!s.isPaused) {
        ...
        // Update Charge Level
        if (s.charging.active) {
            const d = now - s.charging.startTime;
            s.charging.level = Math.min(1, d / 1500); // 0 to 1 for visual glow
        } else {
            s.charging.level = 0;
        }
        ...
        for (let i = s.bullets.length - 1; i >= 0; i--) { 
            ...
            if (b.visualType === 'comet' && s.frame % 2 === 0) {
                s.particles.push({ x: b.x, y: b.y, vx: (Math.random()-0.5)*2, vy: (Math.random()-0.5)*2 + 2, life: 0.5, color: b.beamColor, size: 4 * (b.chargeLevel || 1), type: 'plasma' });
            }
      ...
      s.bullets.forEach(b => { 
          // Scale size and visuals based on charge level
          ...
          if (b.visualType) {
              ...
              if (b.visualType === 'ring') {
                  ...
                  let currY = b.y;
                  let currX = b.x;
                  // More segments if charged
                  const segments = 4 * Math.ceil(chargeScale);
                  for(let k=0; k<segments; k++) {
                      currY -= 12; // Length ~48
                      currX += (Math.random() - 0.5) * 8 * chargeScale;
                      ctx.lineTo(currX, currY);
                  }
                  ctx.stroke();
              }
              else if (b.visualType === 'spindle') {
                  const len = 35 * chargeScale;
                  ctx.fillStyle = clr;
                  ctx.beginPath();
              ...
              if (b.type === 'exotic_arc_hit' && b.target) { ctx.strokeStyle = b.beamColor; ctx.lineWidth = 4 * chargeScale; ctx.beginPath(); ctx.moveTo(s.px, s.py - 35); ctx.lineTo(b.target.x, b.target.y); ctx.stroke(); } 
              else if (b.isBubble) { ctx.strokeStyle = b.beamColor; ctx.lineWidth = 2 * chargeScale; ctx.beginPath(); ctx.arc(b.x, b.y, b.size, 0, Math.PI*2); ctx.stroke(); ctx.fillStyle = b.beamColor + '33'; ctx.fill(); } 
              else if (b.type === 'exotic_plasma_ball') { ctx.fillStyle = b.beamColor; ctx.beginPath(); ctx.arc(b.x, b.y, b.size, 0, Math.PI*2); ctx.fill(); ctx.shadowBlur = 20 * chargeScale; ctx.shadowColor = b.beamColor; ctx.fill(); ctx.shadowBlur = 0; } 
