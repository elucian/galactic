...
export const PaintDialog: React.FC<PaintDialogProps> = ({
  ...
  useEffect(() => {
      if (selectedCustomIndex !== null && gameState.customColors[selectedCustomIndex]) {
          setRgb(hexToRgb(gameState.customColors[selectedCustomIndex]));
      }
  }, [selectedCustomIndex, gameState.customColors]);
    ...
    <div className="fixed inset-0 z-[9950] bg-black/95 flex items-center justify-center p-4 backdrop-blur-md">
        <div className="w-full max-w-5xl bg-zinc-950 border-2 border-zinc-800 rounded-xl overflow-hidden flex flex-col h-[90vh] shadow-2xl">
            ...
            <div className="flex-grow flex flex-col sm:flex-row overflow-hidden">
                <div className="flex-grow p-4 flex flex-col items-center justify-center bg-black/40 border-r border-zinc-900 relative">
                    <ShipIcon 
                ...
                <div className="w-full sm:w-1/3 p-4 flex flex-col gap-4 bg-zinc-900/20 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                            {STANDARD_COLORS.map(c => (
                                <button 
                                    ...
                                    className={`w-full aspect-square rounded border-2 transition-transform hover:scale-105 ${currentColor === c && selectedCustomIndex === null ? 'border-white scale-110 shadow-lg z-10' : 'border-black/20 hover:border-zinc-500'}`} 
                        ...
                        <div className="grid grid-cols-4 gap-2">
                            {gameState.customColors.map((c, i) => (
                                <button 
                                    ...
                                    className={`w-full aspect-square rounded border-2 transition-transform hover:scale-105 ${selectedCustomIndex === i ? 'border-emerald-500 scale-110 shadow-[0_0_10px_#10b981]' : (currentColor === c ? 'border-white' : 'border-black/20 hover:border-zinc-500')}`} 
                                    style={{ backgroundColor: c }} 
                                />
                            ))}
                        </div>
                    ...
                    {selectedCustomIndex !== null && (
                        <div className="space-y-3 pt-4 border-t border-zinc-800 bg-zinc-900/50 p-3 rounded mt-2">
                            <div className="flex justify-between items-center mb-1">
                                <span className="text-[9px] font-black text-emerald-500 uppercase">Spectrum Analyzer</span>
                                <div className="w-4 h-4 rounded border border-zinc-600" style={{ backgroundColor: gameState.customColors[selectedCustomIndex] }} />
                            </div>
