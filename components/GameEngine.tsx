...
const GameEngine: React.FC<GameEngineProps> = ({ ships, shield, secondShield, onGameOver, difficulty, currentPlanet, quadrant }) => {
  ...
  const [stats, setStats] = useState({ 
    ...
    missionTimer: MISSION_DURATION, isPaused: false
  });
  ...
  const stateRef = useRef({
    ...
    isPaused: false,
    pauseStartTime: 0,
    gamePhase: 'travel' as 'travel' | 'observation' | 'boss_intro' | 'boss_fight',
    observationTimer: 0,
    isInitialized: false
  });
  ...
  const applyDamageToEnemy = (en: Enemy, dmg: number, wType?: WeaponType, fromChain: boolean = false) => {
    ...
    if (en.sh > 0) { const leftover = Math.max(0, finalDmg - en.sh); en.sh = Math.max(0, en.sh - finalDmg); if (leftover > 0) en.hp -= leftover; }
      ...
      if (en.hp <= 0) {
        ...
        const scaling = (1 + (difficulty - 1) * 0.1);
        const baseVal = en.type === 'scout' ? 100 : (en.type === 'boss' ? 0 : 300);
        s.score += Math.floor(baseVal * scaling);

        if (en.type === 'boss' && !s.bossDead) { 
          const bossBonus = Math.floor(10000 * Math.pow(1.3, difficulty - 1));
          s.score += bossBonus;
  ...
  const drawShip = (ctx: CanvasRenderingContext2D, sInst: any, x: number, y: number, thrust: number, side: number, isBreaking: boolean, rotation: number = 0, isGhost: boolean = false) => {
    ...
    const drawEngine = (ex: number, ey: number) => {
      ...
      if (thrust > 0) {
        ...
        ctx.lineTo(ex, ey+15+(50 * thrust)+Math.random()*15);
    ...
    // Braking Jets: 2 jets (one left, one right) at the front pushing ship down (firing UP)
    if (isBreaking) {
        ctx.fillStyle = '#f87171'; ctx.globalAlpha = 0.8;
        // Braking jet Left (near nose)
        ctx.beginPath(); ctx.moveTo(30, 20); ctx.lineTo(30, -10); ctx.lineTo(35, 20); ctx.closePath(); ctx.fill();
        // Braking jet Right (near nose)
        ctx.beginPath(); ctx.moveTo(70, 20); ctx.lineTo(70, -10); ctx.lineTo(65, 20); ctx.closePath(); ctx.fill();
        ctx.globalAlpha = 1;
    }

    // Maneuver Jets: Opposite wing fires outward to push ship
    if (side !== 0) {
        ctx.fillStyle = '#38bdf8'; ctx.globalAlpha = 0.9;
        // Move Left (side -1) -> Fire outward jet from RIGHT wing (pushing Left)
        // Move Right (side 1) -> Fire outward jet from LEFT wing (pushing Right)
        const wx = side < 0 ? 85 : 15; 
        const sDir = side < 0 ? 1 : -1; // Firing direction relative to wing

        ctx.beginPath();
        ctx.moveTo(wx, 55);
        ctx.lineTo(wx + (sDir * 35), 55); // Horizontal outward fire
        ctx.lineTo(wx, 60);
        ctx.closePath(); ctx.fill();
        ctx.globalAlpha = 1;
    }
  ...
  useEffect(() => {
    ...
    const resize = () => { 
        ...
        if (s.isInitialized && s.gameActive && !s.playerDead && !s.isPaused) {
            s.isPaused = true;
            s.pauseStartTime = Date.now();
            s.keys.clear(); 
            setStats(p => ({ ...p, isPaused: true }));
        }
    };
    
    const handleBlur = () => {
        const s = stateRef.current;
        if (s.gameActive && !s.playerDead && !s.isPaused) {
            s.isPaused = true;
            s.pauseStartTime = Date.now();
            s.keys.clear(); 
            setStats(p => ({ ...p, isPaused: true }));
    ...
    const loop = () => {
      ...
      if (!s.isPaused) {
        ...
        const missionRemaining = Math.max(0, MISSION_DURATION - missionElapsed);
        const canSpawn = missionElapsed > 10; 

        if (s.gamePhase === 'travel') {
            if (missionRemaining <= 0) {
                s.gamePhase = 'observation';
                s.observationTimer = 0;
                setStats(p => ({ ...p, alert: "SECTOR SUN DETECTED - ARRIVING" }));
                if (!s.sunSpawned) { s.spaceSystems.push(new SpaceSystem(canvas.width, canvas.height, quadrant, currentPlanet.id)); s.sunSpawned = true; }
            }
        } else if (s.gamePhase === 'observation') {
            s.observationTimer++;
            if (s.observationTimer > 360) {
        ...
        if (!s.playerDead) {
            ...
            if (shield && s.sh1 < shield.capacity) {
                ...
                if (!isBroken || timeSinceShatter > 10000) {
                    ...
                    if (s.energy >= regenCost) { s.sh1 = Math.min(shield.capacity, s.sh1 + regenVal); s.energy -= regenCost; if (isBroken && s.sh1 > 0) setStats(p => ({ ...p, alert: "PRIMARY SHIELD REBOOTED" })); }
                }
            }
            if (secondShield && s.sh2 < secondShield.capacity) {
                ...
                if (!isBroken || timeSinceShatter > 10000) {
                    ...
                    if (s.energy >= regenCost) { s.sh2 = Math.min(secondShield.capacity, s.sh2 + regenVal); s.energy -= regenCost; if (isBroken && s.sh2 > 0) setStats(p => ({ ...p, alert: "SECONDARY SHIELD REBOOTED" })); }
                }
            }
            ...
            if (s.isMeltdown) { s.fuel -= 0.005; if (s.frame % 2 === 0) s.particles.push({ x: s.px + (Math.random()-0.5)*30, y: s.py + (Math.random()-0.5)*30, vx: (Math.random()-0.5)*4, vy: 2 + Math.random()*4, life: 1.0, color: '#f97316', size: 8 + Math.random()*12, type: 'fire' }); if (s.fuel <= 0) { s.playerDead = true; createExplosion(s.px, s.py, true); audioService.playExplosion(0, 3.5); s.shake = 95; } }
        ...
        if (canSpawn && s.asteroids.length < maxAst && now - s.lastAsteroidSpawn > astInterval) { s.asteroids.push(new Asteroid(Math.random() * canvas.width, -180, difficulty, s.scavengeMode)); s.lastAsteroidSpawn = now; }
        ...
        if (canSpawn && s.gamePhase === 'travel' && now - s.lastSpawn > spawnTimer) { s.enemies.push(new Enemy(Math.random() * (canvas.width - 120) + 60, -150, Math.random() > 0.3 ? 'fighter' : 'scout', SHIPS[Math.floor(Math.random() * SHIPS.length)], difficulty)); s.lastSpawn = now; }
            ...
            if (m.y < -200 || m.y > canvas.height + 200) s.mines.splice(i, 1);
                ...
                for (let j = s.enemies.length - 1; j >= 0; j--) {
                    ...
                    if (Math.sqrt((m.x - en.x)**2 + (m.y - en.y)**2) < 60) {
                        ...
                        if (en.hp <= 0) { if (en.type === 'boss' && !s.bossDead) { s.bossDead = true; spawnBossExplosions(en.x, en.y); } s.enemies.splice(j, 1); }
                        break;
                    }
                }
            }
        ...
        for (let j = s.enemies.length - 1; j >= 0; j--) {
            ...
            if (s.enemies[j] && !s.playerDead) {
                ...
                if (s.sh2 > 0 && d < 105) { createExplosion(en.x, en.y, false, false, true); s.enemies.splice(j, 1); s.sh2 -= 150; if (s.sh2 <= 0) { s.sh2 = 0; s.sh2ShatterTime = now; } audioService.playShieldHit(); audioService.playExplosion(); continue; } 
                else if (s.sh1 > 0 && d < 85) { createExplosion(en.x, en.y, false, false, true); s.enemies.splice(j, 1); s.sh1 -= 150; if (s.sh1 <= 0) { s.sh1 = 0; s.sh1ShatterTime = now; } audioService.playShieldHit(); audioService.playExplosion(); continue; } 
        ...
        for (let i = s.asteroids.length - 1; i >= 0; i--) { 
            ...
            if (!s.playerDead) { 
                const dx = a.x - s.px, dy = a.y - s.py; const d = Math.sqrt(dx*dx + dy*dy); 
                if (d < a.size + 40 && a.y > 0) { 
                    const activePurple = (shield?.id === 'sh_omega' && s.sh1 > 0) || (secondShield?.id === 'sh_omega' && s.sh2 > 0);
                    const activeRed = (shield?.id === 'sh_beta' && s.sh1 > 0) || (secondShield?.id === 'sh_beta' && s.sh2 > 0);
                    if (activePurple || activeRed) { const bounceSpeed = activePurple ? 1.5 : 1.0; a.vx = (dx/d) * bounceSpeed; a.vy = (dy/d) * bounceSpeed; if (s.sh2 > 0) { s.sh2 -= activePurple ? 15 : 20; if (s.sh2 <= 0) { s.sh2 = 0; s.sh2ShatterTime = now; } } else if (s.sh1 > 0) { s.sh1 -= activePurple ? 15 : 20; if (s.sh1 <= 0) { s.sh1 = 0; s.sh1ShatterTime = now; } } audioService.playShieldHit(); } 
                    else { if (s.sh2 > 0) { s.sh2 -= 60; if (s.sh2 <= 0) { s.sh2 = 0; s.sh2ShatterTime = now; } a.vx = (dx/d)*0.5; a.vy = (dy/d)*0.5; } else if (s.sh1 > 0) { s.sh1 -= 60; if (s.sh1 <= 0) { s.sh1 = 0; s.sh1ShatterTime = now; } a.vx = (dx/d)*0.5; a.vy = (dy/d)*0.5; } else { s.integrity -= 20; s.shake = 40; spawnAsteroidLoot(a.x, a.y, a.variant); s.asteroids.splice(i, 1); createExplosion(a.x, a.y); audioService.playExplosion(); continue; } }
                } 
            } 
        ...
        for (let i = s.gifts.length - 1; i >= 0; i--) { 
            ...
            if (Math.abs(g.x-s.px) < 55 && Math.abs(g.y-s.py) < 55) { 
                ...
                if (g.type === 'energy') { s.energy = maxEnergy * 0.8; setStats(p => ({ ...p, alert: "RESTORED" })); audioService.playSfx('buy'); }
                    ...
                    else s.missionCargo.push({ instanceId: Math.random().toString(36).substr(2, 9), type: cType, id: g.id, name: g.name || g.type.replace('_', ' ').toUpperCase(), weight: 5, quantity: 1 });
                    setStats(p => ({ ...p, alert: `SECURED: ${g.type.toUpperCase()}` })); audioService.playSfx('click');
                }
        ...
        for (let i = s.missiles.length - 1; i >= 0; i--) { 
            ...
            if (!m.target || m.target.hp <= 0) { 
                ...
                if (m.isEmp) { s.enemies.forEach(en => { if (en.sh > 0) { const d = Math.sqrt((m.x-en.x)**2+(m.y-en.y)**2); if (d < md && d < 1200) { md = d; target = en; } } }); }
                if (!target) { md = 25000; s.enemies.forEach(en => { const d = Math.sqrt((m.x-en.x)**2+(m.y-en.y)**2); if(d<md){md=d; target=en;} }); }
                m.target = target; 
            } 
            if (m.target) { const dx = m.target.x - m.x, dy = m.target.y - m.y, d = Math.sqrt(dx*dx+dy*dy); m.vx += (dx/d)*3.8; m.vy += (dy/d)*3.8; } else m.vy -= 1.0; 

            s.asteroids.forEach(ast => { const adx = m.x - ast.x, ady = m.y - ast.y; const distSq = adx*adx + ady*ady; const safetyRad = ast.size + 100; if (distSq < safetyRad*safetyRad) { const d = Math.sqrt(distSq); const force = (1 - (d/safetyRad)) * 2.5; m.vx += (adx/d) * force; m.vy += (ady/d) * force; } });
            m.x += m.vx; m.y += m.vy; m.vx *= 0.96; m.vy *= 0.96; m.life--; 
            
            if (m.life <= 0 || m.y < -700) { createExplosion(m.x, m.y, false, true); s.missiles.splice(i, 1); continue; } 
            ...
            for (let j = s.enemies.length - 1; j >= 0; j--) { 
                if (Math.sqrt((m.x-s.enemies[j].x)**2 + (m.y-s.enemies[j].y)**2) < 80) { 
                    ...
                    if (s.enemies[j].hp <= 0) { if (s.enemies[j].type === 'boss' && !s.bossDead) { s.bossDead = true; spawnBossExplosions(s.enemies[j].x, s.enemies[j].y); } s.enemies.splice(j, 1); } 
                    hitTarget = true; break; 
                } 
            } 
            if (!hitTarget) {
                for (let k = s.asteroids.length - 1; k >= 0; k--) {
                    const ast = s.asteroids[k]; if (ast.y > 0 && Math.sqrt((m.x - ast.x)**2 + (m.y - ast.y)**2) < ast.size + 25) { createImpactEffect(m.x, m.y, 'asteroid'); ast.hp -= m.damage; createExplosion(m.x, m.y); s.missiles.splice(i, 1); if (ast.hp <= 0) { spawnAsteroidLoot(ast.x, ast.y, ast.variant); s.asteroids.splice(k, 1); } hitTarget = true; break; }
                }
            }
        }
        ...
        for (let i = s.bullets.length - 1; i >= 0; i--) { 
            ...
            if (b.type === 'exotic_arc_hit' && b.target) { b.x = b.target.x; b.y = b.target.y; } 
                ...
                if (target) { const dx = target.x - b.x, dy = target.y - b.y, d = Math.sqrt(dx*dx+dy*dy); b.vx += (dx/d)*2.5; b.vy += (dy/d)*2.5; } b.y += b.vy; b.x += b.vx;
            }
            ...
            for (let j = s.enemies.length - 1; j >= 0; j--) { if (Math.abs(b.x - s.enemies[j].x) < 70 && Math.abs(b.y - s.enemies[j].y) < 70) { createImpactEffect(b.x, b.y, s.enemies[j].sh > 0 ? 'shield' : 'vessel', s.enemies[j].sh > 0 ? (s.enemies[j].type === 'boss' ? '#a855f7' : '#38bdf8') : undefined); applyDamageToEnemy(s.enemies[j], b.damage, b.wType); hit = true; break; } } 
            if (!hit) { for (let j = s.asteroids.length - 1; j >= 0; j--) { const ast = s.asteroids[j]; if (ast.y > 0 && Math.abs(b.x - ast.x) < ast.size + 30 && Math.abs(b.y - ast.y) < ast.size + 30) { createImpactEffect(b.x, b.y, 'asteroid'); let aDmg = b.damage; if (b.type === 'exotic_mining_laser') aDmg *= 5.0; ast.hp -= aDmg; if (ast.hp <= 0) { spawnAsteroidLoot(ast.x, ast.y, ast.variant); s.asteroids.splice(j, 1); } hit = true; break; } } } 
        ...
        for (let i = s.enemyBullets.length - 1; i >= 0; i--) { 
            ...
            if (!s.playerDead && Math.abs(eb.x - s.px) < 60 && Math.abs(eb.y - s.py) < 60) { s.enemyBullets.splice(i, 1); let dmg = eb.isExotic ? 20 : 10; createImpactEffect(eb.x, eb.y, (s.sh1 > 0 || s.sh2 > 0) ? 'shield' : 'vessel', (s.sh2 > 0 ? secondShield?.color : (s.sh1 > 0 ? shield?.color : undefined))); if (s.sh2 > 0) { s.sh2 -= dmg * 1.5; if (s.sh2 <= 0) { s.sh2 = 0; s.sh2ShatterTime = now; } } else if (s.sh1 > 0) { s.sh1 -= dmg * 1.5; if (s.sh1 <= 0) { s.sh1 = 0; s.sh1ShatterTime = now; } } else s.integrity -= dmg; audioService.playShieldHit(); continue; } 
            for (let j = s.asteroids.length - 1; j >= 0; j--) { const ast = s.asteroids[j]; if (ast.y > 0 && Math.abs(eb.x - ast.x) < ast.size + 20 && Math.abs(eb.y - ast.y) < ast.size + 20) { createImpactEffect(eb.x, eb.y, 'asteroid'); ast.hp -= 40; s.enemyBullets.splice(i, 1); if (ast.hp <= 0) { spawnAsteroidLoot(ast.x, ast.y, ast.variant); s.asteroids.splice(j, 1); } break; } }
        ...
        if (s.sunSpawned) {
             ...
             sys.viewProgress = Math.min(1, sys.viewProgress + 0.003);
             sys.planets.forEach(p => { p.angle += p.speed; if (p.moon) p.moon.angle += p.moon.speed; });
             sys.comets.forEach(c => { c.angle += c.speed; c.x = sys.x + Math.cos(c.angle) * c.a; c.y = sys.y + Math.sin(c.angle) * c.b; const dx = c.x - sys.x, dy = c.y - sys.y; const dist = Math.sqrt(dx*dx + dy*dy); c.tailLength = Math.max(0, 150 - (dist / 10)); });
        }
        ...
        const mRemaining = Math.max(0, MISSION_DURATION - ((now - s.missionStartTime) / 1000));
      ...
      s.spaceSystems.forEach(sys => { 
        ctx.save(); 
        const glow = ctx.createRadialGradient(sys.x, sys.y, 0, sys.x, sys.y, sys.sunSize * 5); glow.addColorStop(0, sys.sunColor + '66'); glow.addColorStop(1, 'transparent'); ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(sys.x, sys.y, sys.sunSize*5, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = sys.sunColor; ctx.beginPath(); ctx.arc(sys.x, sys.y, sys.sunSize, 0, Math.PI*2); ctx.fill(); 
        sys.comets.forEach(c => { const dx = c.x - sys.x, dy = c.y - sys.y; ctx.save(); ctx.translate(c.x, c.y); const angleToSun = Math.atan2(dy, dx); const tailGrad = ctx.createLinearGradient(0, 0, Math.cos(angleToSun)*c.tailLength, Math.sin(angleToSun)*c.tailLength); tailGrad.addColorStop(0, '#fff'); tailGrad.addColorStop(0.5, '#38bdf844'); tailGrad.addColorStop(1, 'transparent'); ctx.strokeStyle = tailGrad; ctx.lineWidth = 4; ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(Math.cos(angleToSun)*c.tailLength, Math.sin(angleToSun)*c.tailLength); ctx.stroke(); ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(0,0,3,0,Math.PI*2); ctx.fill(); ctx.restore(); });
        sys.planets.forEach(p => { const px = sys.x + Math.cos(p.angle)*p.distance; const py = sys.y + Math.sin(p.angle)*p.distance; const currentSize = 4 + (p.baseSize - 4) * sys.viewProgress; ctx.fillStyle = p.color; ctx.beginPath(); ctx.arc(px, py, currentSize, 0, Math.PI*2); ctx.fill(); if (p.isSelected) { ctx.strokeStyle = '#fff'; ctx.lineWidth = 1; ctx.setLineDash([3,3]); ctx.beginPath(); ctx.arc(px, py, currentSize + 5, 0, Math.PI*2); ctx.stroke(); ctx.setLineDash([]); } if (p.moon) { const mx = px + Math.cos(p.moon.angle)*p.moon.distance; const my = py + Math.sin(p.moon.angle)*p.moon.distance; ctx.fillStyle = '#9ca3af'; ctx.beginPath(); ctx.arc(mx, my, p.moon.size, 0, Math.PI*2); ctx.fill(); } }); 
        ctx.restore(); 
      });
      ...
      s.gifts.forEach(g => { ctx.save(); ctx.translate(g.x, g.y); ctx.rotate(s.frame*0.08); const boxSize = 18; ctx.fillStyle = g.type === 'weapon' ? '#a855f7' : (g.type === 'energy' ? '#00f2ff' : (g.type === 'gold' ? '#fbbf24' : (g.type === 'platinum' ? '#e2e8f0' : (g.type === 'lithium' ? '#c084fc' : (g.type === 'repair' ? '#10b981' : '#60a5fa'))))); ctx.fillRect(-boxSize/2, -boxSize/2, boxSize, boxSize); ctx.strokeStyle = '#fff'; ctx.lineWidth = 1; ctx.strokeRect(-boxSize/2, -boxSize/2, boxSize, boxSize); ctx.fillStyle = '#000'; ctx.font = 'bold 11px Arial'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; let letter = ''; if (g.type === 'gold') letter = 'G'; else if (g.type === 'platinum') letter = 'P'; else if (g.type === 'lithium') letter = 'L'; else if (g.type === 'missile') letter = 'S'; else if (g.type === 'mine') letter = 'M'; else if (g.type === 'fuel') letter = 'F'; else if (g.type === 'energy') letter = 'E'; else if (g.type === 'weapon') letter = 'W'; else if (g.type === 'repair') letter = 'H'; ctx.fillText(letter, 0, 0); ctx.restore(); });
      
      if (activeShip) {
        let currentThrust = 0.5; // persistent small jet
        let breaking = false;
        if (s.keys.has('KeyW') || s.keys.has('ArrowUp')) currentThrust = 1.3; // Accel
        else if (s.keys.has('KeyS') || s.keys.has('ArrowDown')) { currentThrust = 0; breaking = true; } // Decel -> zero main jet, breaking active
        
        let side = 0;
        if (s.keys.has('KeyA') || s.keys.has('ArrowLeft')) side = -1;
        else if (s.keys.has('KeyD') || s.keys.has('ArrowRight')) side = 1;
        
        drawShip(ctx, activeShip, s.px, s.py, currentThrust, side, breaking, 0, s.playerDead);
      }
