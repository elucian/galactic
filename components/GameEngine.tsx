...
// VERSION: V80.0 - Ordnance & HUD Overhaul
...
const GameEngine: React.FC<GameEngineProps> = ({ ships, shield, secondShield, onGameOver, difficulty }) => {
  ...
  const createExplosion = (x: number, y: number, isBoss: boolean = false, isMine: boolean = false) => {
    ...
    const count = isBoss ? 100 : (isMine ? 60 : 30);
    for (let i = 0; i < count; i++) {
      const angle = Math.random() * Math.PI * 2;
      const speed = isBoss ? Math.random() * 14 : (isMine ? Math.random() * 10 : Math.random() * 7);
      s.particles.push({
        x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
        life: 1.0 + Math.random() * 0.5, size: Math.random() * (isBoss ? 10 : (isMine ? 8 : 5)) + 2,
        color: isBoss ? (Math.random() > 0.4 ? '#f97316' : '#ffffff') : (isMine ? '#10b981' : (Math.random() > 0.7 ? '#ffffff' : '#f59e0b')),
        type: 'fire'
      });
      if (isBoss || isMine) {
        s.particles.push({
          x, y, vx: Math.cos(angle) * (speed * 0.4), vy: Math.sin(angle) * (speed * 0.4),
          life: 2.0, size: Math.random() * 25 + 15, color: isMine ? 'rgba(16, 185, 129, 0.2)' : 'rgba(50, 50, 50, 0.4)', type: 'smoke'
        });
      }
    }
  };
  ...
  useEffect(() => {
    ...
    let lastW = window.innerWidth || 1024;
    let lastH = window.innerHeight || 768;
    ...
    const resize = () => { 
      ...
      if (stateRef.current.px === 0) {
        ...
        const ratioX = lastW > 0 ? stateRef.current.px / lastW : 0.5;
        const ratioY = lastH > 0 ? stateRef.current.py / lastH : 0.85;
    ...
    const handleInputReset = () => stateRef.current.keys.clear();
    window.addEventListener('keydown', (e) => handleKey(e, true));
    window.addEventListener('keyup', (e) => handleKey(e, false));
    window.addEventListener('blur', handleInputReset);
    window.addEventListener('focus', handleInputReset);
    ...
    const loop = () => {
      ...
      if ((s.keys.has('Space') || s.keys.has('KeyF')) && Date.now() - s.lastFire > 110 && s.energy > 12 && s.equippedWeapons.length > 0) {
        s.equippedWeapons.forEach((w, idx) => {
          ...
          if (weaponDef.id === 'exotic_wave') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vy: -15, damage: weaponDef.damage, type: 'wave', t: s.frame, beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_bolt') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vy: -20, damage: weaponDef.damage, type: 'bolt', beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_beam') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vy: -25, damage: weaponDef.damage, type: 'beam', beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_spiral') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vy: -12, damage: weaponDef.damage, type: 'spiral', t: s.frame, beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_split') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vx: -4, vy: -18, damage: weaponDef.damage, type: 'std', beamColor: weaponDef.beamColor }); s.bullets.push({ x: s.px + xOff, y: s.py - 35, vx: 0, vy: -18, damage: weaponDef.damage, type: 'std', beamColor: weaponDef.beamColor }); s.bullets.push({ x: s.px + xOff, y: s.py - 35, vx: 4, vy: -18, damage: weaponDef.damage, type: 'std', beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_saw') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vy: -10, damage: weaponDef.damage, type: 'saw', t: s.frame, beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_grav') { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vy: -6, damage: weaponDef.damage, type: 'grav', beamColor: weaponDef.beamColor }); } 
          else if (weaponDef.id === 'exotic_flame') { for(let i=0; i<3; i++) s.bullets.push({ x: s.px + xOff + (Math.random()-0.5)*15, y: s.py - 35, vx: (Math.random()-0.5)*4, vy: -15 - Math.random()*10, damage: weaponDef.damage/3, type: 'fire', life: 25, beamColor: weaponDef.beamColor }); } 
          else { s.bullets.push({ x: s.px + xOff, y: s.py - 35, vx: 0, vy: -18.5, damage: weaponDef.damage, type: 'std', beamColor: weaponDef.beamColor }); }
        });
      ...
      for (let i = s.particles.length - 1; i >= 0; i--) { const p = s.particles[i]; p.x += p.vx; p.y += p.vy; p.life -= 0.025; if (p.type === 'smoke') p.size += 0.3; if (p.life <= 0) s.particles.splice(i, 1); }
            ...
            if (Math.sqrt((m.x - en.x)**2 + (m.y - en.y)**2) < 70) {
                ...
                s.enemies.forEach(otherEn => {
                  ...
                  if (dist < AOE_RADIUS) {
                    ...
                    applyDamageToEnemy(otherEn, m.damage * falloff);
                    if (otherEn.hp <= 0 && otherEn !== en) { /* Side kills handled by radial loop logic */ }
                  }
                });
                s.mines.splice(i, 1); createExplosion(m.x, m.y, false, true); audioService.playExplosion(0, 1.6);
                // Clean up dead enemies after AoE
      ...
      for (let i = s.gifts.length - 1; i >= 0; i--) {
        ...
        if (Math.abs(g.x - s.px) < 45 && Math.abs(g.y - s.py) < 45) {
          ...
          if (g.type === 'ammo') {
            ...
            s.mineStock = Math.min(50, s.mineStock + 15); // UPDATED: Cap to 50
            setStats(p => ({ ...p, alert: "AMMO RESTOCKED" }));
          } else if (g.type === 'weapon' && g.id) {
            const newWeps = [...s.equippedWeapons]; newWeps[0] = { id: g.id, count: 1 }; s.equippedWeapons = newWeps;
      ...
      for (let i = s.enemyBullets.length - 1; i >= 0; i--) { const eb = s.enemyBullets[i]; eb.y += eb.vy; if (Math.abs(eb.x - s.px) < 38 && Math.abs(eb.y - s.py) < 38) { s.enemyBullets.splice(i, 1); if (s.sh2 > 0) s.sh2 -= 50; else if (s.sh1 > 0) s.sh1 -= 50; else s.hp -= 25; audioService.playShieldHit(); } if (eb.y > canvas.height + 150) s.enemyBullets.splice(i, 1); }
      ...
      if (s.bossDead && s.enemies.length === 0 && s.gifts.length === 0) { s.gameActive = false; onGameOverRef.current(true, s.score, false, { rockets: s.missileStock, mines: s.mineStock, weapons: s.equippedWeapons }); return; }
      ...
      s.particles.forEach(p => { ctx.fillStyle = p.color; ctx.globalAlpha = p.life; if (p.type === 'smoke') { ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI*2); ctx.fill(); } else { ctx.fillRect(p.x - p.size/2, p.y - p.size/2, p.size, p.size); } ctx.globalAlpha = 1.0; });
      ...
      s.mines.forEach(m => { ctx.save(); ctx.translate(m.x, m.y); ctx.beginPath(); ctx.arc(0, 0, 12, 0, Math.PI*2); ctx.fillStyle = '#00ff88'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke(); ctx.restore(); });
      ...
      if (s.sh1 > 0) { ctx.strokeStyle = '#00ffff'; ctx.lineWidth = 3.5; ctx.setLineDash([14, 8]); ctx.beginPath(); ctx.arc(s.px, s.py, 72, Math.PI*1.1, Math.PI*1.9); ctx.stroke(); ctx.setLineDash([]); }
      if (s.sh2 > 0) { ctx.strokeStyle = '#ff0000'; ctx.lineWidth = 7; ctx.beginPath(); ctx.arc(s.px, s.py, 92, Math.PI*1.1, Math.PI*1.9); ctx.stroke(); }
      ctx.restore();
      anim = requestAnimationFrame(loop);
    };
    anim = requestAnimationFrame(loop);
    return () => { cancelAnimationFrame(anim); window.removeEventListener('resize', resize); window.removeEventListener('keydown', handleKey as any); window.removeEventListener('keyup', handleKey as any); window.removeEventListener('blur', handleInputReset); window.removeEventListener('focus', handleInputReset); };
  ...
  return (
    <div className="w-full h-full bg-black relative overflow-hidden">
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-end gap-2.5 z-50 pointer-events-none opacity-90 scale-90">
        <div className="flex flex-col items-center gap-1.5">
          ...
          <div className="flex flex-col-reverse gap-0.5 p-1 bg-zinc-950/60 border border-zinc-800/40 rounded backdrop-blur-sm">
            {Array.from({ length: nl }).map((_, i) => (<div key={i} className={`w-3.5 h-1 rounded-xs transition-colors duration-300 ${i < afl ? 'shadow-[0_0_5px_currentColor]' : 'opacity-10'}`} style={{ backgroundColor: i < afl ? (currentFuel<1?'#ff0055':'#0099ff') : '#18181b', color: currentFuel<1?'#ff0055':'#0099ff' }} />))}
          </div>
        ...
        <div className="flex flex-col items-center gap-1.5">
          ...
          <div className="flex flex-col-reverse gap-0.5 p-1 bg-zinc-950/60 border border-zinc-800/40 rounded backdrop-blur-sm">
            {Array.from({ length: nl }).map((_, i) => { const r = i/nl, c = ep<25 ? '#ff0000' : (r<0.3 ? '#ff3300' : (r<0.6 ? '#ffff00' : '#00ffff')); return (<div key={i} className={`w-3.5 h-1 rounded-xs transition-colors duration-300 ${i < ael ? 'shadow-[0_0_5px_currentColor]' : 'opacity-10'}`} style={{ backgroundColor: i < ael ? c : '#18181b', color: c }} />); })}
          </div>
      ...
      {stats.boss && (
        <div className="absolute top-14 left-1/2 -translate-x-1/2 w-[350px] flex flex-col items-center gap-1 z-50 pointer-events-none bg-transparent p-3 rounded-lg border border-white/5 backdrop-blur-[1px] opacity-90">
          <div className="retro-font text-[6px] text-purple-400 uppercase tracking-[0.4em] font-black drop-shadow-[0_0_5px_#a855f7]">XENOS PRIMARY</div>
          <div className="w-full flex flex-col gap-1 mt-1.5">
            {stats.boss.sh > 0 && (<div className="w-full h-1.5 bg-zinc-900/40 border border-purple-900/30 rounded-full overflow-hidden"><div className="h-full bg-purple-500 shadow-[0_0_10px_#a855f7]" style={{ width: `${(stats.boss.sh/stats.boss.maxSh)*100}%` }} /></div>)}
            <div className="w-full h-2 bg-zinc-900/40 border border-red-900/30 rounded-full overflow-hidden"><div className="h-full bg-red-600 shadow-[0_0_10px_#dc2626]" style={{ width: `${(stats.boss.hp/stats.boss.maxHp)*100}%` }} /></div>
          </div>
        </div>
      )}

      <div className="absolute top-3 left-5 flex flex-col gap-2.5 pointer-events-none z-50 opacity-95 scale-90 origin-top-left">
        <div className="flex items-center gap-2.5"><div className="retro-font text-[6px] text-lime-400 uppercase w-8 font-black drop-shadow-[0_0_3px_#a3e635]">HULL</div><div className="w-40 h-1.5 bg-zinc-950/40 border border-zinc-800/40 rounded-full overflow-hidden"><div className="h-full bg-lime-500 shadow-[0_0_8px_#84cc16]" style={{ width: `${stats.hp}%` }} /></div></div>
        {shield && <div className="flex items-center gap-2.5"><div className="retro-font text-[6px] uppercase w-8 font-black" style={{ color: shield.color, textShadow: `0 0 5px ${shield.color}` }}>SHLD</div><div className="w-40 h-1.5 bg-zinc-950/40 border border-zinc-800/40 rounded-full overflow-hidden"><div className="h-full shadow-[0_0_8px_currentColor]" style={{ width: `${(stats.sh1/shield.capacity)*100}%`, backgroundColor: shield.color }} /></div></div>}
      </div>

      <div className="absolute top-3 right-5 text-right pointer-events-none z-50 flex flex-col gap-1 opacity-80 scale-90 origin-top-right"><div className="retro-font text-[20px] text-white tabular-nums drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">{stats.score.toLocaleString()}</div><div className="retro-font text-[6px] text-zinc-400 uppercase tracking-widest font-black">UNITS</div></div>

      <div className="absolute bottom-3 left-5 flex flex-col gap-3 pointer-events-none p-3 bg-zinc-950/20 border border-white/5 rounded-lg min-w-[180px] backdrop-blur-[1px] opacity-90 scale-85 origin-bottom-left">
         <div className="flex flex-col gap-1.5">
           <span className="retro-font text-[6px] text-red-400 uppercase tracking-widest font-black drop-shadow-[0_0_3px_#f87171]">MISSIL</span>
           <div className="grid grid-cols-10 gap-1.5 w-fit">
             {Array.from({ length: stats.missiles }).map((_, i) => (<div key={i} className="w-1.5 h-1.5 rounded-xs bg-red-500 shadow-[0_0_5px_#ef4444]" />))}
           </div>
         </div>
         <div className="flex flex-col gap-1.5">
           <span className="retro-font text-[6px] text-lime-400 uppercase tracking-widest font-black drop-shadow-[0_0_3px_#a3e635]">MINES</span>
           <div className="grid grid-cols-10 gap-1.5 w-fit">
             {Array.from({ length: stats.mines }).map((_, i) => (<div key={i} className="w-1.5 h-1.5 rounded-full bg-lime-500 shadow-[0_0_5px_#84cc16]" />))}
           </div>
         </div>
      </div>
    </div>
