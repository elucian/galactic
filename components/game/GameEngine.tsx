            ...
            if (['missile', 'missile_emp', 'mine', 'mine_emp', 'mine_red', 'missile_enemy', 'mine_enemy'].includes(b.type)) {
                ...
                if (age < 20 && !b.isEnemy) { 
                    ...
                    if (b.isEnemy) {
                        if (!s.rescueMode) {
                            if (b.type === 'mine_enemy') {
                                // MINE PHYSICS:
                                // 0-1s (60 frames): Free drift (inertial)
                                // 1s-10s (60-600 frames): Magnetic Homing
                                // >10s: Target Lost (Drift + Explode)
                                if (age < 60) {
                                    // Inertial drift phase (initial velocity)
                                } else if (age < 600) {
                                    // Magnetic Homing Phase
                                    if (age === 60) { 
                                        b.color = '#ef4444'; // Turns red when active
                                        audioService.playSfx('click');
                                    }
                                    
                                    // Steering Behavior: Seek Player with Momentum
                                    const targetX = s.px;
                                    const targetY = s.py;
                                    ...
                                    if (dist > 0) {
                                        const desiredSpeed = 8; // Max mine speed
                                        const steeringForce = 0.15; // Low force = wide turns / elliptical misses
                                        
                                        const desiredVx = (dx / dist) * desiredSpeed;
                                        const desiredVy = (dy / dist) * desiredSpeed;
                                        
                                        // Reynolds Steering: Velocity += Steering Force
                                        b.vx += (desiredVx - b.vx) * steeringForce;
                                        b.vy += (desiredVy - b.vy) * steeringForce;
                                    }
                                } else {
                                    // Timeout Phase: Target Lost
                                    // Explode immediately as per spec "explode when 10 second expired"
                                    // Or trigger standard expiry logic via life=0
                                    b.life = 0; 
                                }
                    ...
                    if (isMissile || b.type === 'mine_enemy') { 
                        ...
                        // Mines accelerate if homing
                        if (b.type === 'mine_enemy' && age > 60 && age < 600 && currentSpeed < (b.maxSpeed || 9)) {
                            b.vx *= 1.02; b.vy *= 1.02;
                        }
                    }
                }
