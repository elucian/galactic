...
const App: React.FC = () => {
  const createInitialState = (): GameState => {
    ...
    initialOwned.forEach((os) => { 
      ...
      initialFittings[os.instanceId] = { weapons: [{ id: DEFAULT_WEAPON_ID, count: 1 }], shieldId: null, secondShieldId: null, flareId: null, reactorLevel: 1, engineType: 'standard', rocketCount: 0, mineCount: 0, wingWeaponId: null, health: 100, ammoPercent: 100, lives: 1, fuel: config.maxFuel }; 
  ...
  useEffect(() => {
    if (isStoreOpen && gameState.selectedShipInstanceId) {
      const inst = gameState.ownedShips.find(o => o.instanceId === gameState.selectedShipInstanceId);
      if (inst) setInspectedShipId(inst.shipTypeId);
    }
  }, [isStoreOpen, gameState.selectedShipInstanceId, gameState.ownedShips]);
  ...
  const handleGameOver = useCallback((success: boolean, finalScore: number, wasAborted?: boolean, payload?: { rockets: number, mines: number, weapons: EquippedWeapon[], fuel: number, bossDefeated?: boolean, health?: number }) => {
    setGameState(prev => {
  ...
  const refuelSelected = () => {
    if (!selectedFitting || !gameState.selectedShipInstanceId || !selectedShipConfig || selectedFitting.health <= 0 || selectedFitting.fuel >= selectedShipConfig.maxFuel || gameState.credits < REFUEL_COST_PER_UNIT) { audioService.playSfx('denied'); return; }
    const max = selectedShipConfig.maxFuel;
    const nextAmount = Math.min(max, selectedFitting.fuel + 1.0);
    setGameState(p => ({ ...p, credits: p.credits - REFUEL_COST_PER_UNIT, shipFittings: { ...p.shipFittings, [p.selectedShipInstanceId!]: { ...selectedFitting, fuel: nextAmount } } }));
    audioService.playSfx('buy');
  };

  const repairSelected = () => {
    if (!selectedFitting || !gameState.selectedShipInstanceId || selectedFitting.health >= 100 || selectedFitting.health <= 0 || gameState.credits <= 0) { audioService.playSfx('denied'); return; }
    const neededPercent = 100 - selectedFitting.health, fullCost = Math.floor(neededPercent * REPAIR_COST_PER_PERCENT);
    if (gameState.credits >= fullCost) setGameState(p => ({ ...p, credits: p.credits - fullCost, shipFittings: { ...p.shipFittings, [p.selectedShipInstanceId!]: { ...selectedFitting, health: 100 } } }));
    else { const added = gameState.credits / REPAIR_COST_PER_PERCENT; setGameState(p => ({ ...p, credits: 0, shipFittings: { ...p.shipFittings, [p.selectedShipInstanceId!]: { ...selectedFitting, health: Math.min(100, selectedFitting.health + added) } } })); }
    audioService.playSfx('buy');
  };
  ...
  const removeWeaponFromSlot = (slotIdx: number) => {
    ...
    setGameState(p => { const newWeps = [...selectedFitting.weapons]; newWeps[slotIdx] = undefined as any; return { ...p, credits: p.credits + (oldW ? oldW.price : 0), shipFittings: { ...p.shipFittings, [sId]: { ...selectedFitting, weapons: newWeps.filter(Boolean) } } }; });
    audioService.playSfx('click');
  };
    ...
    <div className="w-full h-full bg-[#0c0a09] text-zinc-100 flex flex-col overflow-hidden relative">
        ...
        <div className="fixed inset-0 z-[9500] bg-black/95 flex items-center justify-center p-6 backdrop-blur-md">
           <div className="w-full max-w-xl bg-zinc-950 border-2 border-zinc-800 rounded-xl overflow-hidden flex flex-col shadow-2xl">
              ...
              <div className="p-4 space-y-4">
                 ...
                 <div className="space-y-3">
                    <label className="text-[9px] uppercase font-black text-zinc-500 tracking-widest">Pilot Dossier</label>
                    <div className="space-y-4 bg-zinc-900/40 p-4 rounded-lg border border-zinc-800/50">
                       <div className="flex flex-col gap-1.5">
                          <span className="text-[9px] font-black uppercase text-zinc-500">Fleet Callsign</span>
                          <input type="text" maxLength={12} value={gameState.pilotName} onChange={e => setGameState(p => ({ ...p, pilotName: e.target.value.toUpperCase() }))} className="w-full bg-zinc-950 border border-zinc-800 p-2 text-[11px] font-black uppercase text-emerald-400 focus:outline-none focus:border-emerald-500 transition-colors" placeholder="CALLSIGN..." />
                       </div>
                       <div className="flex flex-col gap-2">
                          <span className="text-[9px] font-black uppercase text-zinc-500">Identity Selection</span>
                          <div className="grid grid-cols-5 gap-2">
                             {AVATARS.map(a => (
                                <button key={a.label} onClick={() => setGameState(p => ({ ...p, pilotAvatar: a.icon }))} className={`aspect-square flex items-center justify-center text-2xl rounded border transition-all ${gameState.pilotAvatar === a.icon ? 'bg-emerald-500/20 border-emerald-500' : 'bg-zinc-950 border-zinc-800 opacity-60'}`}>{a.icon}</button>
                             ))}
                          </div>
                       </div>
                    </div>
              ...
              <footer className="p-4 bg-zinc-900/50 border-t border-zinc-800">
                 <button onClick={() => setIsOptionsOpen(false)} className="w-full py-3 bg-emerald-600 text-white font-black uppercase text-[9px] tracking-widest rounded shadow active:scale-95 transition-all">SYNCHRONIZE SYSTEMS</button>
              </footer>
           </div>
        </div>
      )}
      ...
      {screen === 'hangar' && (
        <div className="flex-grow flex flex-col p-6 space-y-6 z-10 overflow-hidden">
            ...
            {gameState.ownedShips.map((instance, idx) => {
              ...
              return (
                <div key={idx} onClick={() => setGameState(p => ({ ...p, selectedShipInstanceId: instance.instanceId }))} className={`flex-grow border-2 p-6 flex flex-col items-center rounded-xl transition-all cursor-pointer ${isSelected ? 'border-emerald-500 bg-emerald-950/20 shadow-xl' : 'border-zinc-800 bg-zinc-950/40 opacity-70 hover:opacity-100'}`}>
                  <div className="w-full flex justify-between items-center mb-4"><span className="retro-font text-[9px] text-emerald-500 uppercase">{config.name}</span><span className="text-[8px] uppercase text-zinc-500">Fleet Slot {idx + 1}</span></div>
                  <ShipIcon config={config as any} hullColor={gameState.shipColors[instance.instanceId]} wingColor={gameState.shipWingColors[instance.instanceId]} cockpitColor={gameState.shipCockpitColors[instance.instanceId]} gunColor={gameState.shipGunColors[instance.instanceId]} gunBodyColor={gameState.shipGunBodyColors[instance.instanceId]} engineColor={gameState.shipEngineColors[instance.instanceId]} nozzleColor={gameState.shipNozzleColors[instance.instanceId]} className="w-44 h-44" />
                  <div className="w-full mt-auto space-y-3">
                    <div className="space-y-1"><div className="flex justify-between text-[8px] uppercase"><span>Hull Integrity</span><span className={f.health <= 0 ? "text-red-500 font-bold" : ""}>{f.health <= 0 ? "TOTALED" : `${Math.floor(f.health)}%`}</span></div><div className="h-2 bg-zinc-900 rounded-full overflow-hidden"><div className={`h-full ${f.health <= 0 ? 'bg-red-600' : 'bg-emerald-500'}`} style={{ width: `${f.health}%` }} /></div></div>
                    <div className="space-y-1"><div className="flex justify-between text-[8px] uppercase"><span>Fuel Reserve</span><span>{f.fuel.toFixed(1)} Units</span></div><div className="h-2 bg-zinc-900 rounded-full overflow-hidden"><div className="h-full bg-blue-500" style={{ width: `${(f.fuel/config.maxFuel)*100}%` }} /></div></div>
                    <div className="flex flex-col gap-1 border-t border-zinc-800/40 pt-2"><div className="flex justify-between text-[7px] uppercase font-black"><span className="text-zinc-500">Defense PRI:</span><span className={priS ? "text-emerald-400" : "text-zinc-700"}>{priS ? priS.name : 'OFFLINE'}</span></div><div className="flex justify-between text-[7px] uppercase font-black"><span className="text-zinc-500">Defense SEC:</span><span className={secS ? "text-red-400" : "text-zinc-700"}>{secS ? secS.name : 'OFFLINE'}</span></div></div>
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <button onClick={(e) => { e.stopPropagation(); setIsStoreOpen(true); }} className="py-2 bg-zinc-900 text-[8px] uppercase font-black rounded hover:bg-zinc-800 transition-colors">SHIPYARD</button>
                      <button 
                        onClick={(e) => { e.stopPropagation(); setIsLoadoutOpen(true); }} 
                        disabled={f.health <= 0}
                        className="py-2 bg-zinc-900 text-[8px] uppercase font-black rounded hover:bg-zinc-800 transition-colors disabled:opacity-30 disabled:grayscale">LOADOUT</button>
                    </div>
                  </div>
                </div>
              );
          ...
          <footer className="bg-zinc-950/90 p-5 rounded-xl border-2 border-zinc-800 flex justify-between items-center backdrop-blur-md shadow-2xl">
            <div className="flex gap-10 items-center">
               <div onClick={() => setScreenState('intro')} className="flex items-center gap-4 bg-zinc-900/60 p-2 pr-6 rounded-lg border border-zinc-800 shadow-inner group hover:border-emerald-500/30 transition-colors cursor-pointer">
                  <div className="w-12 h-12 flex items-center justify-center text-3xl bg-zinc-950 rounded border border-zinc-700 group-hover:border-emerald-500/50 transition-colors">{gameState.pilotAvatar}</div>
                  <div className="flex flex-col"><span className="text-[7px] uppercase font-black text-zinc-500">Authorized Pilot</span><span className="text-[12px] font-black uppercase text-emerald-400 tracking-widest">{gameState.pilotName}</span></div>
               </div>
               <div className="flex flex-col gap-1"><span className="retro-font text-[9px] text-emerald-500 uppercase tracking-tighter">BUDGET TERMINAL</span><span className="text-emerald-400 font-black text-sm tabular-nums shadow-[0_0_10px_rgba(16,185,129,0.5)]">${Math.floor(gameState.credits).toLocaleString()} RESOURCES</span></div>
            </div>
            <div className="flex gap-4">
               <button onClick={repairSelected} disabled={!selectedFitting || selectedFitting.health >= 100 || selectedFitting.health <= 0 || gameState.credits <= 0} className="px-6 py-3 bg-zinc-900 border border-zinc-700 text-[10px] uppercase font-black hover:bg-zinc-800 disabled:opacity-30 transition-all rounded shadow-sm">REPAIR</button>
               <button onClick={refuelSelected} disabled={!selectedFitting || !selectedShipConfig || selectedFitting.health <= 0 || selectedFitting.fuel >= selectedShipConfig.maxFuel || gameState.credits < REFUEL_COST_PER_UNIT} className="px-6 py-3 bg-zinc-900 border border-zinc-700 text-[10px] uppercase font-black hover:bg-zinc-800 disabled:opacity-30 transition-all rounded shadow-sm">REFUEL</button>
               <button onClick={() => setIsPaintOpen(true)} disabled={!gameState.selectedShipInstanceId || (selectedFitting && selectedFitting.health <= 0)} className="px-6 py-3 bg-zinc-900 border border-zinc-700 text-[10px] uppercase font-black hover:bg-zinc-800 transition-all rounded shadow-sm">PAINT</button>
               
               {isStalemate ? (
                 <button onClick={restartGame} className="px-12 py-3 bg-red-600 border-2 border-red-400 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-red-500/20 active:scale-95 transition-all rounded-md animate-pulse">REBOOT</button>
               ) : (
                 <button onClick={() => setScreenState('map')} disabled={!selectedFitting || selectedFitting.fuel <= 0 || selectedFitting.health <= 0 || selectedFitting.weapons.length === 0} className="px-12 py-3 bg-emerald-600 border-2 border-emerald-400 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all rounded-md disabled:opacity-30 disabled:grayscale">ENGAGE</button>
               )}
            </div>
          </footer>
        </div>
      ...
      {isPaintOpen && gameState.selectedShipInstanceId && selectedShipConfig && (
        <div className="fixed inset-0 z-[8500] bg-black/95 flex items-center justify-center p-6 backdrop-blur-md">
           <div className="w-full max-w-5xl bg-zinc-950 border-2 border-zinc-800 rounded-xl overflow-hidden flex flex-col h-[85vh] shadow-2xl">
              <header className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50"><div className="flex gap-8"><h2 className="retro-font text-emerald-500 text-xs uppercase">Advanced Paint System</h2><div className="flex gap-4">{['hull', 'wings', 'cockpit', 'guns', 'gun_body', 'engines', 'nozzles'].map(p => (<button key={p} onClick={() => setActivePart(p as any)} className={`text-[9px] uppercase font-black transition-all ${activePart === p ? 'text-emerald-400 underline decoration-2 underline-offset-4' : 'text-zinc-500 hover:text-white'}`}>{p.replace('_', ' ')}</button>))}</div></div><button onClick={() => setIsPaintOpen(false)} className="px-4 py-2 text-red-500 uppercase font-black text-[9px] hover:bg-red-500/10">CLOSE BAY</button></header>
              <div className="flex-grow flex overflow-hidden">
                 <div className="w-2/3 p-12 flex flex-col items-center justify-center relative bg-black/40 border-r border-zinc-900"><div className="absolute top-6 left-6 text-[8px] text-emerald-500/40 uppercase tracking-widest font-black">INTERACTIVE PREVIEW - CLICK PART TO SELECT</div><ShipIcon config={selectedShipConfig} className="w-96 h-96 drop-shadow-[0_0_80px_rgba(16,185,129,0.1)]" activePart={activePart} onPartSelect={(p) => setActivePart(p)} hullColor={gameState.shipColors[gameState.selectedShipInstanceId]} wingColor={gameState.shipWingColors[gameState.selectedShipInstanceId]} cockpitColor={gameState.shipCockpitColors[gameState.selectedShipInstanceId]} gunColor={gameState.shipGunColors[gameState.selectedShipInstanceId]} gunBodyColor={gameState.shipGunBodyColors[gameState.selectedShipInstanceId]} engineColor={gameState.shipEngineColors[gameState.selectedShipInstanceId]} nozzleColor={gameState.shipNozzleColors[gameState.selectedShipInstanceId]} showJets={true} /></div>
                 <div className="w-1/3 p-8 flex flex-col gap-8 bg-zinc-900/20"><div className="space-y-2"><label className="text-[10px] font-black uppercase text-zinc-400">Target Segment</label><div className="text-xl font-black text-white uppercase tracking-tighter">{activePart.replace('_', ' ')}</div></div><div className="space-y-4"><label className="text-[10px] font-black uppercase text-zinc-400">Materials & Finishes</label><div className="grid grid-cols-5 gap-3">{['#94a3b8', '#ef4444', '#f59e0b', '#10b981', '#3b82f6', '#8b5cf6', '#ec4899', '#ffffff', '#171717', '#4ade80', '#00f2ff', '#fbbf24', '#f87171', '#a855f7', '#d946ef'].map(c => (<button key={c} onClick={() => setPartColor(c)} className={`w-full aspect-square rounded border-2 transition-all hover:scale-110 active:scale-90 ${ (gameState[('ship' + activePart.charAt(0).toUpperCase() + activePart.slice(1) + 'Colors') as keyof GameState]?.[gameState.selectedShipInstanceId!] === c) ? 'border-white' : 'border-black/20'}`} style={{ backgroundColor: c }} />))}</div></div><div className="mt-auto p-4 bg-emerald-500/5 border border-emerald-500/20 rounded"><p className="text-[9px] text-emerald-400 leading-relaxed uppercase font-black">System Note: Segmented painting uses no resources. Changes apply instantly to tactical systems.</p></div></div>
              </div>
           </div>
        </div>
      )}
      ...
      {isLoadoutOpen && selectedShipConfig && selectedFitting && (
        <div className="fixed inset-0 z-[7000] bg-black/90 flex items-center justify-center p-6 backdrop-blur-xl">
           <div className="w-full max-w-4xl bg-zinc-950 border-2 border-zinc-800 rounded-xl overflow-hidden flex flex-col h-[85vh] shadow-2xl">
              <header className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50"><div className="flex gap-4 items-center"><div className="flex gap-2">{['guns', 'ordnance', 'defense'].map(t => (<button key={t} onClick={() => setLoadoutTab(t as any)} className={`px-5 py-3 text-[10px] font-black uppercase border-b-2 transition-all ${loadoutTab === t ? 'border-emerald-500 text-emerald-400 bg-emerald-500/5' : 'border-transparent text-zinc-500'}`}>{t}</button>))}</div><div className="ml-8 px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 rounded flex flex-col justify-center"><span className="text-[7px] uppercase font-black text-emerald-500/60 leading-none mb-1">Resources Available</span><span className="text-[11px] font-black text-emerald-400 leading-none tabular-nums shadow-emerald-500/20 shadow-sm">${Math.floor(gameState.credits).toLocaleString()}</span></div></div><button onClick={() => setIsLoadoutOpen(false)} className="px-8 py-2 bg-emerald-600/10 border border-emerald-500 text-emerald-500 uppercase font-black text-[10px] hover:bg-emerald-600 hover:text-white transition-all rounded">DONE</button></header>
              {loadoutTab === 'defense' && (<div className="p-4 bg-zinc-900/80 border-b border-zinc-800"><div className="flex justify-center gap-16 px-10"><div className="flex flex-col"><span className="text-[7px] uppercase font-black text-zinc-500">PRIMARY SYSTEM</span><span className="text-[10px] uppercase font-black text-emerald-400">{selectedFitting.shieldId ? SHIELDS.find(x => x.id === selectedFitting.shieldId)?.name : 'OFFLINE'}</span></div><div className="flex flex-col"><span className="text-[7px] uppercase font-black text-zinc-500">SECONDARY SYSTEM</span><span className="text-[10px] uppercase font-black text-red-400">{selectedFitting.secondShieldId ? SHIELDS.find(x => x.id === selectedFitting.secondShieldId)?.name : 'OFFLINE'}</span></div></div></div>)}
              <div className="p-8 overflow-y-auto flex-grow custom-scrollbar space-y-8">
                {loadoutTab === 'guns' && (<><div className="flex gap-4">{Array.from({ length: selectedShipConfig.defaultGuns }).map((_, i) => (<button key={i} onClick={() => setActiveSlot(i)} className={`flex-grow p-4 text-[9px] uppercase font-black border transition-all rounded ${activeSlot === i ? 'bg-emerald-500/20 border-emerald-500' : 'bg-zinc-900 border-zinc-800 opacity-60'}`}>Hardpoint {i+1}: {selectedFitting.weapons[i] ? [...WEAPONS, ...EXOTIC_WEAPONS].find(w => w.id === selectedFitting.weapons[i].id)?.name : 'EMPTY'}</button>))}</div><div className="space-y-2">{WEAPONS.map(w => { const eqL = selectedFitting.weapons[0]?.id === w.id, eqR = selectedFitting.weapons[1]?.id === w.id; return (<div key={w.id} className="grid grid-cols-[1fr_120px_220px] items-center gap-4 p-4 bg-zinc-900/40 border border-zinc-800 rounded group hover:border-zinc-700 transition-colors"><div className="text-[11px] font-black uppercase group-hover:text-emerald-400">{w.name}</div><div className="text-[11px] text-yellow-500 font-bold tabular-nums">${w.price.toLocaleString()}</div><div className="flex gap-2 justify-end">{selectedShipConfig.defaultGuns >= 1 && (<button onClick={() => eqL ? removeWeaponFromSlot(0) : mountWeaponToSlot(w, 0)} className={`flex-grow py-2 text-[9px] font-black uppercase rounded border transition-all ${eqL ? 'border-red-500 text-red-500 hover:bg-red-500 hover:text-white' : 'border-emerald-500/30 text-emerald-500 hover:bg-emerald-600 hover:text-white'}`}>{eqL ? 'REMV L' : 'MNT L'}</button>)}{selectedShipConfig.defaultGuns >= 2 && (<button onClick={() => eqR ? removeWeaponFromSlot(1) : mountWeaponToSlot(w, 1)} className={`flex-grow py-2 text-[9px] font-black uppercase rounded border transition-all ${eqR ? 'border-red-500 text-red-500 hover:bg-red-500 hover:text-white' : 'border-emerald-500/30 text-emerald-500 hover:bg-emerald-600 hover:text-white'}`}>{eqR ? 'REMV R' : 'MNT R'}</button>)}</div></div>); })}</div></>)}
                {loadoutTab === 'ordnance' && (<div className="space-y-6"><div className="grid grid-cols-2 gap-6"><div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded text-center"><div className="text-[8px] text-zinc-500 mb-2 uppercase">MISSILE STOCK</div><div className="text-2xl font-black text-white tabular-nums">{selectedFitting.rocketCount} / {MAX_MISSILES}</div></div><div className="p-6 bg-zinc-900/50 border border-zinc-800 rounded text-center"><div className="text-[8px] text-zinc-500 mb-2 uppercase">MINE STOCK</div><div className="text-2xl font-black text-white tabular-nums">{selectedFitting.mineCount} / {MAX_MINES}</div></div></div><div className="space-y-2">{EXPLODING_ORDNANCE.map(o => { const isM = o.id.includes('missile'), curr = isM ? selectedFitting.rocketCount : selectedFitting.mineCount, max = isM ? MAX_MISSILES : MAX_MINES; return (<div key={o.id} className="grid grid-cols-[1fr_120px_150px] items-center gap-4 p-4 bg-zinc-900/40 border border-zinc-800 rounded group hover:border-zinc-700 transition-colors"><div className="text-[11px] font-black uppercase group-hover:text-emerald-400">{o.name.replace('Rockets', 'Missiles')} (+10)</div><div className="text-[11px] text-yellow-500 font-bold tabular-nums">${o.price.toLocaleString()}</div><button onClick={() => buyAmmo(isM ? 'missile' : 'mine')} disabled={curr >= max} className="py-3 bg-red-600/10 border border-red-500/30 text-red-500 text-[9px] font-black uppercase rounded hover:bg-red-600 hover:text-white disabled:opacity-20 transition-all">LOAD +10</button></div>); })}</div></div>)}
                {loadoutTab === 'defense' && (<div className="space-y-2">{SHIELDS.map(s => { const eq1 = selectedFitting.shieldId === s.id, eq2 = selectedFitting.secondShieldId === s.id; return (<div key={s.id} className="grid grid-cols-[1fr_120px_240px] items-center gap-4 p-4 bg-zinc-900/40 border border-zinc-800 rounded group hover:border-zinc-700 transition-colors"><div className="text-[11px] font-black uppercase group-hover:text-emerald-400">{s.name}</div><div className="text-[11px] text-yellow-500 font-bold tabular-nums">${s.price.toLocaleString()}</div><div className="flex gap-2 justify-end"><button onClick={() => eq1 ? removeShield(1) : equipShield(s, 1)} className={`flex-grow py-2 text-[8px] font-black uppercase rounded border transition-all ${eq1 ? 'bg-blue-600 border-blue-400' : 'border-blue-500/30 text-blue-500 hover:bg-blue-600/10'}`}>{eq1 ? 'REMV PRI' : 'PRIMARY'}</button><button onClick={() => eq2 ? removeShield(2) : equipShield(s, 2)} className={`flex-grow py-2 text-[8px] font-black uppercase rounded border transition-all ${eq2 ? 'bg-red-600 border-red-400' : 'border-red-500/30 text-red-500 hover:bg-red-600/10'}`}>{eq2 ? 'REMV SEC' : 'SECONDARY'}</button></div></div>); })}</div>)}
              </div>
           </div>
        </div>
      )}

      {isStoreOpen && (<div className="fixed inset-0 z-[8000] bg-black/95 flex items-center justify-center p-6 backdrop-blur-sm"><div className="w-full max-w-4xl bg-zinc-950 border-2 border-zinc-800 rounded-xl overflow-hidden flex flex-col h-[80vh] shadow-2xl"><header className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50"><div className="flex items-center gap-6"><h2 className="retro-font text-emerald-500 text-xs uppercase">SHIPYARD</h2><div className="px-4 py-1.5 bg-emerald-500/10 border border-emerald-500/30 rounded flex items-center gap-3"><span className="text-[7px] uppercase font-black text-emerald-500/60">WALLET</span><span className="text-[11px] font-black text-emerald-400 tabular-nums">${Math.floor(gameState.credits).toLocaleString()} RESOURCES</span></div></div><button onClick={() => setIsStoreOpen(false)} className="text-zinc-500 hover:text-white uppercase font-black text-[9px]">CANCEL</button></header><div className="flex-grow flex overflow-hidden"><div className="w-1/3 border-r border-zinc-800 overflow-y-auto p-2 space-y-2">{SHIPS.map((s) => (<button key={s.id} onClick={() => setInspectedShipId(s.id)} className={`w-full p-4 text-left flex flex-col gap-1 rounded border transition-all ${inspectedShipId === s.id ? 'bg-emerald-500/10 border-emerald-500 shadow-lg' : 'bg-zinc-900 border-zinc-800 opacity-70'}`}><span className="text-[10px] font-black uppercase text-white">{s.name}</span><span className="text-[9px] font-black text-yellow-500 tabular-nums">${s.price.toLocaleString()}</span></button>))}</div><div className="flex-grow p-12 flex flex-col items-center justify-center bg-zinc-950/40 text-center"><ShipIcon config={SHIPS.find(s => s.id === inspectedShipId) || SHIPS[0]} className="w-64 h-64 mb-10" /><h3 className="retro-font text-sm text-emerald-500 uppercase mb-4">{SHIPS.find(s => s.id === inspectedShipId)?.name}</h3><div className="mb-8 space-y-1"><p className="text-[10px] uppercase text-zinc-500">{SHIPS.find(s => s.id === inspectedShipId)?.description}</p><p className="text-[10px] uppercase text-blue-400 font-bold">FUEL CAPACITY: {SHIPS.find(s => s.id === inspectedShipId)?.maxFuel} UNITS</p></div><button onClick={() => replaceShip(inspectedShipId)} disabled={gameState.credits < (SHIPS.find(s => s.id === inspectedShipId)?.price || 0)} className="w-full max-w-xs py-5 bg-emerald-600 border-2 border-emerald-400 text-white font-black uppercase rounded shadow-lg active:scale-95 transition-all disabled:opacity-20 disabled:grayscale">AUTHORIZE REPLACEMENT</button></div></div></div></div>)}
      {screen === 'map' && (<div className="flex-grow flex flex-col p-6 space-y-6 z-10"><header className="flex justify-between items-center bg-zinc-950/90 p-5 border-2 border-zinc-800 rounded-xl"><div className="flex gap-4">{Object.values(QuadrantType).map(q => (<button key={q} onClick={() => setGameState(p => ({ ...p, currentQuadrant: q, currentPlanet: null }))} className={`px-5 py-2 text-[10px] font-black uppercase rounded border transition-all ${gameState.currentQuadrant === q ? 'bg-emerald-600 text-white border-emerald-400' : 'bg-zinc-900 border-zinc-800 text-zinc-500'}`}>{q} SECTOR</button>))}</div><button onClick={() => setScreenState('hangar')} className="px-8 py-2 bg-zinc-900 border border-zinc-700 text-[10px] uppercase font-black rounded-lg hover:bg-zinc-800 transition-colors">RETURN TO BASE</button></header><div className="flex-grow flex flex-wrap items-center justify-center gap-16">{PLANETS.filter(p => p.quadrant === gameState.currentQuadrant).map(p => (<div key={p.id} onClick={() => setGameState(prev => ({ ...prev, currentPlanet: p }))} className={`group p-8 border-2 rounded-xl flex flex-col items-center transition-all cursor-pointer ${gameState.currentPlanet?.id === p.id ? 'border-emerald-500 bg-emerald-500/10 scale-110 shadow-xl' : 'border-zinc-800/40 hover:border-zinc-700 opacity-60 hover:opacity-100'}`}><div className="w-24 h-24 rounded-full mb-6 shadow-2xl transition-transform group-hover:rotate-6" style={{ backgroundColor: p.color, backgroundImage: `radial-gradient(circle at 30% 30%, rgba(255,255,255,0.2), transparent)` }} /><p className="text-[12px] font-black uppercase text-white mb-2 tracking-widest">{p.name}</p><p className="text-[9px] uppercase text-zinc-500 font-black">Threat Level {p.difficulty}</p></div>))}</div><div className="flex justify-center h-20">{gameState.currentPlanet && (<button onClick={() => setScreenState('game')} disabled={!selectedFitting || selectedFitting.fuel <= 0 || selectedFitting.health <= 0 || selectedFitting.weapons.length === 0} className="px-20 py-6 bg-emerald-600 border-2 border-emerald-400 text-white text-[14px] font-black uppercase tracking-[0.4em] rounded-xl shadow-xl hover:scale-105 active:scale-95 transition-all disabled:opacity-30 disabled:grayscale">ENGAGE</button>)}</div></div>)}
      {screen === 'game' && gameState.selectedShipInstanceId && (<div className="flex-grow relative overflow-hidden z-20"><GameEngine ships={activeShipsForEngine as any} shield={SHIELDS.find(s => s.id === gameState.shipFittings[gameState.selectedShipInstanceId!]?.shieldId) || null} secondShield={SHIELDS.find(s => s.id === gameState.shipFittings[gameState.selectedShipInstanceId!]?.secondShieldId) || null} difficulty={gameState.currentPlanet?.difficulty || 1} onGameOver={handleGameOver} /></div>)}
